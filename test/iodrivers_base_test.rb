...
describe OroGen.iodrivers_base.Task do
    ...
    describe "the behavior of FD-driven activities" do
        describe "with fd-based communication" do
            attr_reader :subject_task
        ...
        describe "with port-based communication" do
            attr_reader :subject_task
            ...
            it "calls processIO repeatedly until all data has been read" do
                syskit_configure_and_start(@subject_task)
                packets = (0...24).to_a.each_slice(2).map do |b|
                    raw_packet_from_s(b.pack("c*"))
                end
    ...
    describe "the behavior on triggered (e.g. port_driven) activities" do
        before do
            @subject_task = syskit_deploy(
                OroGen.iodrivers_base.test.PortDriven
                      .deployed_as("#{Process.pid}-#{name}")
            )
        end
    ...
    describe "the behavior on periodic activities" do
        before do
            @subject_task = syskit_deploy(
